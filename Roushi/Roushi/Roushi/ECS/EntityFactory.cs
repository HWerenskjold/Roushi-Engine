...
namespace Roushi
{
    class EntityFactory
    {
        public static Entity CreatePlayer(int health, int damage, int speed,  Vector2 position, string spriteName, Vector2 frameSize)
        {
            Entity player = new Entity();
            player.Attach(new StatsPart(health, damage));
        ...
        public static Entity KindSign(string text, string fontType, Vector2 position, Vector2 textboxPosition, Color textColor, Color bgColor, Boolean drawBackground, string backgroundName)
        {
            ...
            dummy.Attach(new CollisionPart());
            dummy.Get<CollisionPart>().OverrideCollisionBox(new Rectangle((int)position.X - dummy.Get<DrawablePart>().GetBoundingBox().Width/2, (int)position.Y, dummy.Get<DrawablePart>().GetBoundingBox().Width, dummy.Get<DrawablePart>().GetBoundingBox().Height));
            dummy.Attach(new TalkToPart(text, fontType, textboxPosition, textColor, bgColor, drawBackground, backgroundName));

            return dummy;
        }

        public static Entity CreateWorm(Vector2 position)
        {
            Entity dummy = new Entity();
            dummy.categoryID = "insect";
            dummy.uniqueID = "worm";
            dummy.Attach(new TransformPart(position));
            dummy.Attach(new DrawablePart("NPC/Insect/Worm/worm", new Vector2(16, 8), DepthLayer.Stage));
            dummy.Attach(new CollisionPart());
            //dummy.Attach(new FleeAI());
